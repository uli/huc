...
*****************************************************************************/
void convert_period(int period,
		    char* alpha,
		    char* octave)
{
 int index;
  ...
  */
  ...
  while ((index < NB_OCTAVE * NOTE_PER_OCTAVE) && (ft_period[index]>period))
    index+= NOTE_PER_OCTAVE;

  if ((index == 0) && (ft_period[index] < period))
    { /* period too low */
      nb_warning++;
  ...
  if (index >= NB_OCTAVE * NOTE_PER_OCTAVE)
    { /* period too high */
      nb_warning++;
...
*****************************************************************************/
void handle_note_mml(int period,
		     int instrument,
		     int effect_id,
		     int effect_data)
{
...
*/
 ...
 switch (effect_id)
 {
  case FX_VOLUME:
	...
	nb_warning++;
	log_warning("Volume too high (0X%02X > 0X40)", effect_data);
      }
    else
	    ...
	    unsigned char opcode)
{
  if (channel[channel_number].last_note != 0)
    {
      int duration = convert_row_to_duration(channel[channel_number].last_note) - 1;

#if DEBUG > 1
      static unsigned long total_duration_elapsed[8] = {0,0,0,0,0,0,0,0};
...
*****************************************************************************/
void gen_d2z_instrument(FILE* f,
			int channel_number)
{
			 ...
			 int effect_data)
{
 ...
 switch (effect_id)
 {
  case FX_VOLUME:
	...
	((effect_data >> 1 > 31) && (channel[current_channel].volume == 31)))
  ...
  case FX_VOLUME_SLIDE:
    {
      effect_data = ((effect_data >> 4) & 0xF) - (effect_data & 0xF);
...
void finish_d2z()
{
 ...
 sprintf(output_filename + strlen(output_filename),
  ...
  if (channel[current_channel].last_note != 0)
    {
      int duration = convert_row_to_duration(channel[current_channel].last_note);
...
*****************************************************************************/
void
read_byte(FILE* in, unsigned char* result)
{
 fread(result, 1, 1, in);
}
...
*****************************************************************************/
void
read_byte_array(FILE* in, char* result, int length)
{
...
*****************************************************************************/
void
read_word_motorola(FILE* in, unsigned int* result)
{
...
*****************************************************************************/
void
read_word_intel(FILE* in, unsigned int* result)
{
 unsigned char hi_byte, lo_byte;
...
log_warning (char *format, ...)
{
  ...
  log_raw("Warning (%s, song position %d, row 0X%02X, channel %d) :\n   %s\n",
	  input_filename,
...
*****************************************************************************/
unsigned long
position_in_file(int pattern_number)
{
...
*****************************************************************************/
void
handle_pattern(FILE* in, int pattern_number)
{
  ...
  for (current_row = 0; current_row < ROW_NUMBER_PER_PATTERN; current_row ++)
    {
#if DEBUG > 1
       printf("row %02X\n", current_row);
#endif
...
#if DEBUG > 1
	 printf("channel %d : Note %4d inst %2X FX %1X  data %02X\n",
		current_channel,
		note_instrument & 0xFFF,
		((note_instrument >> 8) & 0xF0) + (instrument_effect >> 4),
		instrument_effect & 0xF,
		effect_data);
#endif
...
int main(int argc, char *argv[])
{
 ...
 if (!strncmp(signature, "M.K.", SIGNATURE_LENGTH))
   { /* Signature is egal to M.K. */
    nb_instrument = 31;
    }
 ...
 for (current_instrument = 0;
      current_instrument < nb_instrument;
      current_instrument ++)
 {
   printf("%d/%d\r", current_instrument + 1, nb_instrument);
#if 0
   convertion_inst[current_instrument] = nearest_pce_instrument(current_instrument);
#endif
 }
 ...
 for (current_song_position = 0;
      ...
      current_song_position ++)
 {
   printf("%d/%d\r", current_song_position + 1, song_length);
   handle_pattern(input,  pattern_array[current_song_position]);
 }
